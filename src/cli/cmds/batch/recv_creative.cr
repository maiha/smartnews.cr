class Cmds::BatchCmd
  ...
  def recv_creative
    ...
    house = house(Smartnews::Proto::Creative)

    # iterate job
    done_count = 0

    insights = house(Smartnews::Proto::Insight).load
    cids = insights.map(&.creative_id.to_s).reject(&.empty?).sort.uniq
    cids.each_with_index do |cid, i|
      room  = house.chdir(File.join(house.dir, "data", cid))
      label = "%s(%s/%s)[%s]" % [hint, i+1, cids.size, cid]

      converter = converter(Smartnews::Converter::Creative)

      recv_creative_impl(room, converter, cid, label)
  ...
  private def recv_creative_impl(house, converter, cid, hint)
